...
def load_model():
    ...
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    # checkpt_path = '/scratch/ws2263/DL25SP-Final-Project-main/checkpoints/jepa_byol.pth'
    ...
    model.load_state_dict(torch.load(config.checkpt_path, map_location=device))
    model.eval()
    return model
